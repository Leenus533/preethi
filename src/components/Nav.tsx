...
export function Wordmark({ className = "" }: { className?: string }) {
  return (
    <Link href="/" className={`focus-ring flex items-center gap-2.5 rounded-lg ${className}`} aria-label={`${SITE.name} home`}>
      <span className="font-display grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-pine-600 to-pine-800 text-lg text-white shadow-[var(--shadow-soft)]">
        P
      </span>
...
export function Nav() {
  return (
    <header className="sticky top-0 z-40 border-b border-cream-200/80 bg-cream-50/85 backdrop-blur-md supports-[backdrop-filter]:bg-cream-50/75">
      <div className="container-x flex h-16 items-center justify-between gap-4">
        ...
        <nav aria-label="Primary" className="hidden items-center gap-1 md:flex">
          <div className="group relative">
            <Link
              href="/#subjects"
              className="focus-ring inline-flex items-center gap-1 rounded-full px-3 py-2 text-sm font-medium text-ink-soft hover:bg-cream-100 hover:text-pine-900"
            >
              Subjects
          ...
          {NAV_LINKS.map((l) => (
            <Link key={l.href} href={l.href} className="focus-ring rounded-full px-3 py-2 text-sm font-medium text-ink-soft hover:bg-cream-100 hover:text-pine-900">
              {l.label}
            </Link>
          ))}
        </nav>
