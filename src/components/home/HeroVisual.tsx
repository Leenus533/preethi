/**
 * Decorative hero composition: a study board with the three subjects Preethi teaches
 * (a maths curve, a benzene ring, a DNA helix) and a mock session card. Pure SVG and CSS,
 * so it ships no image bytes and needs no photograph.
 */
export function HeroVisual({ className = "" }: { className?: string }) {
  return (
    <div className={`relative ${className}`} aria-hidden="true">
      <div className="relative aspect-[5/4] overflow-hidden rounded-[1.75rem] border border-cream-300 bg-gradient-to-br from-pine-50 via-cream-50 to-cream-100 shadow-[var(--shadow-card)] sm:aspect-[4/3] lg:aspect-[4/5]">
        <div className="bg-dots mask-fade absolute inset-0" />
        ...
        {/* Handwritten-style annotation pointing at the vertex. */}
        <div className="absolute bottom-[22%] left-[46%] flex items-end gap-1 sm:bottom-[20%]">
          <svg viewBox="0 0 60 40" className="h-8 w-12 text-clay-700" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
            <path d="M56 6C40 8 22 18 8 34" />
            <path d="M8 34l2-9M8 34l9-2" />
          </svg>
          <span className="font-display -translate-y-4 text-[1.05rem] italic leading-none text-clay-700">this is where it clicks</span>
        </div>

        {/* Benzene ring, top-right. */}
        <svg viewBox="0 0 100 100" className="drift absolute right-[7%] top-[6%] w-[24%] text-pine-700" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinejoin="round">
        ...
        {/* DNA helix, left edge. */}
        <svg viewBox="0 0 100 200" className="drift drift-slow absolute left-[6%] top-[8%] h-[46%] text-pine-500" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round">
        ...
        {/* Session card. */}
        <div className="rise rise-2 absolute right-[6%] top-[34%] w-[62%] max-w-[17rem] rounded-2xl border border-cream-300 bg-white p-4 shadow-[var(--shadow-lift)] sm:top-[30%] lg:top-[36%]">
          <div className="flex items-center justify-between gap-2">
            <span className="text-[0.7rem] font-semibold uppercase tracking-wide text-muted">Next session</span>
            <span className="chip !py-1 !text-[0.68rem]">
          ...
          <p className="mt-0.5 text-[0.8rem] text-ink-soft">Thursday 17:30 to 18:30 · Google Meet</p>
          ...
          <div className="mt-3 border-t border-cream-200 pt-3">
            ...
            <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-cream-200">
              <div className="h-full w-[72%] rounded-full bg-gradient-to-r from-pine-500 to-pine-700" />
            </div>
          </div>
        </div>

        {/* Sticker. */}
        <div className="rise rise-3 absolute right-[10%] bottom-[9%] -rotate-6 rounded-xl border border-clay-200 bg-clay-100 px-3 py-2 shadow-[var(--shadow-soft)] sm:bottom-[8%]">
          <p className="font-display text-[1.35rem] leading-none text-clay-700">A · A · A</p>
          <p className="mt-1 text-[0.65rem] font-medium text-clay-700">Biology, Chemistry, Maths</p>
        </div>
      </div>
    </div>
  );
