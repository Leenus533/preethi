...
4. Optional but recommended: create one product per paid service with `serviceId` metadata matching the ids in `src/lib/config.ts` (`gcse-60`, `alevel-60`, `medicine-60`). Checkout attaches payments to these so reporting groups per service; without them it falls back to ad-hoc products. The live and test accounts both have them.
